...
<template>
  ...
  <option v-for="sectorItem in sectorCategories" > 
    {{sectorItem}}
  </option>
</select>
</div>
<div v-if="sector" class="form-group">
  ...
  <option v-for="subsectorItem in subsectorCategories" > 
    {{subsectorItem}}
  </option>
</select>
</div>
...
computed: {
  ...
  subsectorCategories () {
    return this.sectors.filter(p => p.sector==this.sector).map(s=>s.subsector)
  }
},
  methods: {
 getSectors(){
